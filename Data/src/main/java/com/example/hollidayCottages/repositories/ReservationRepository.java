...
public interface ReservationRepository extends JpaRepository<Reservation, Integer> {
    @Query("SELECT r FROM Reservation r WHERE r.cottageId = :cottageId AND (((r.startDate BETWEEN :startDate AND :endDate) OR (r.endDate BETWEEN :startDate AND :endDate)) OR (:startDate BETWEEN r.startDate AND r.endDate) OR (:endDate BETWEEN r.startDate AND r.endDate))")
